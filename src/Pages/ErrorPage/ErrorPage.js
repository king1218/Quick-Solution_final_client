  ...
  let from = location.state?.from?.pathname || "/";
    return (
        <div>
           ...
           <section class="flex items-center h-screen lg:h-full sm:p-10 dark:bg-gray-900 dark:text-gray-100">
	<div class="container flex flex-col items-center justify-center px-5 mx-auto my-8 space-y-8 text-center sm:max-w-md">
		...
		<p class="lg:text-3xl text-2xl f1 text-indigo-700">{error}</p>
        <button onClick={()=> navigate(from, { replace: true })} className='btn btn-link f1 text-2xl'>Go Back</button>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="w-20 h-20 text-indigo-700 dark:text-indigo-700">
